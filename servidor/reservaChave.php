...
<html lang="pt-br">
  ...
  <nav class="navbar navbar-light bg-light">
    ...
    <section id="formulario" style="height: 380px;">
      <form action="confirmaReserva.php" method="POST">
        <fieldset>
            <legend>Reservar chave:</legend>
            <div>
              <label for="matricula">Matrícula:</label><br>
              <input type="text" id="matricula" name="matricula" value='<?php echo$_SESSION['user']?>' required>
                  
              <label for="chave">Escolha a chave que deseja reservar:</label><br>
              <select name="chave" id="chave" required>
                <option></option>
                <?php
                  $sql = "select * from chave";
                  $resultado2 = $conexao -> query($sql);
                  while($key_data = mysqli_fetch_assoc($resultado2)){
                  ?>
                    <option value='<?php echo$key_data['num_chave']?>'><?php echo $key_data["descricao"] ?></option>
                  <?php 
                  }
                  $conexao -> close();
                  ?>
              ...
              <label for="turno">Escolha o turno que deseja reservar:</label>
        ...
        <button class="formButton" type="submit">Reservar</button>
      </form>
    </section>
